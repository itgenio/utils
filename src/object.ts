...
export const getEnumKeys = <T extends {}>(object: T): (keyof T)[] => {
  return Object.keys(object).filter(key => !(parseInt(key) >= 0)) as (keyof T)[];
};
...
export const getKeyFromNumericEnum = <T extends {}>(object: T, value: T[keyof T]) => {
  return Object.keys(object).find(
    key => ((object[(value as unknown) as keyof T] as unknown) as keyof T) === (key as keyof T)
 ...
 */
export const clearFalsyValues = <T extends {}>(object: T, falsyValues: unknown[] = [null, '', undefined]): T => {
  const obj = {} as T;
...
export const initEnumRecord = <
  T extends Object,
  V extends F extends (K: keyof T) => infer R ? (unknown extends R ? number : R) : F,
 ...
 * @return {number}
 */
export const sumEnumRecord = <T extends Record<string, number>>(obj: T): number => {
...
export const flatObject = <T extends {}>(obj: T): T => {
  const newObject: any = {};
  const path: string[] = [];
